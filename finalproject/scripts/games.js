...
function createGameCard(gamesArray, sectionId, headingText) {
...
gamesArray.forEach(beginnerGame => {
    const div = document.createElement('div');
    const img = document.createElement('img');
    img.src = beginnerGame.imageUrl;
    img.alt = beginnerGame.game;
    img.loading = "lazy";
    ...
    div.appendChild(p1);
    div.appendChild(p2);
    h4.textContent = beginnerGame.game;
    p1.textContent = `Players: ${beginnerGame.info}`;
    p2.textContent = `Description: ${beginnerGame.description}`;
    div.classList.add(`game-card`);
   });
main.appendChild(section);
}

createGameCard(beginnerGames, 'Beginner-games', 'Beginner');
createGameCard(intermediateGames, 'Intermediate-games', 'Intermediate');
createGameCard(advancedGames, 'Advance-games', 'Advanced');
